...
function pixResultHtml(payment) { if (!payment) return ''; const qr = payment.qrCodeBase64 ? `<img class="pix-result-qr" src="data:image/png;base64,${payment.qrCodeBase64}" alt="QR Code Pix">` : ''; const code = payment.qrCopyPaste ? `<textarea id="successPixCode" readonly>${safe(payment.qrCopyPaste)}</textarea><button class="secondary-btn" id="copySuccessPixBtn" type="button">Copiar Pix copia e cola</button>` : ''; return `<div class="pix-result"><strong id="paymentStatusText">Pagamento Pix pendente</strong><p id="paymentStatusHelp">Escaneie o QR Code ou copie o código abaixo para pagar.</p>${qr}${code}</div>`; }
async function watchPayment(paymentId) { if (!paymentId) return; const started = Date.now(); const timer = setInterval(async () => { if (Date.now() - started > 30 * 60 * 1000) return clearInterval(timer); const { data, error } = await db.functions.invoke('hyper-api', { body: { action: 'status', id: paymentId } }); const status = String(data?.status || '').toLowerCase(); if (!error && ['approved', 'paid', 'completed'].includes(status)) { clearInterval(timer); const statusEl = $('#paymentStatusText'), helpEl = $('#paymentStatusHelp'); if (statusEl) statusEl.textContent = 'Pagamento confirmado ✅'; if (helpEl) helpEl.textContent = 'Recebemos o pagamento. Seu pedido está confirmado.'; } }, 5000); }
...
$('#checkoutForm').addEventListener('submit', async event => {
  ...
  if (dominipayPayment?.id) watchPayment(dominipayPayment.id);
