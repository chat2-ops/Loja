...
async function createDominipayPayment(order) { const { data, error } = await db.functions.invoke('create-dominipay-payment', { body: { amount: order.total, observation: `Pedido ${order.id}` } }); if (error) throw error; if (!data || !data.id) throw new Error(data?.error || 'A Dominipay não retornou o pagamento.'); return data; }
...
$('#clearCartBtn').addEventListener('click', () => { cart = {}; renderCart(); }); $('#checkoutBtn').addEventListener('click', prepareCheckout);
...
$('#checkoutForm').addEventListener('submit', async event => {
  ...
  cart = {}; renderCart(); $('#checkoutForm').hidden = true; $('#orderSuccess').hidden = false;
  ...
  $('#newOrderBtn').addEventListener('click', () => { $('#checkoutForm').reset(); $('#checkoutForm').hidden = false; $('#orderSuccess').hidden = true; closeModal('checkoutModal'); });
});
